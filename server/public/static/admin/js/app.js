 ...
 * @param select 选择器
 */
function saveNum(url, select) {
    var data = {};
    $(select || ".goods-input").each(function (index, item) {
        data[$(item).attr('data-id')] = $(item).val();
    });
    if (data.length == 0) {
        layer.msg('没有对象');
    ...
    ajax_req(url || THINK_CONTROLLER + '/saveNum', {'rest_num': data});
}
/**
 * 保存其他说明
 *
 ...
 * @param select 选择器
 */
function sendGift(url, select) {
    var data = {};
    $(select || ".gift-input").each(function (index, item) {
        data[$(item).attr('data-id')] = $(item).val();
    });
    if (data.length == 0) {
        layer.msg('没有对象');
    ...
    ajax_req(url || THINK_CONTROLLER + '/sendGift', {'other': data});
}
/**
