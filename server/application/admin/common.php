...
function get_type($id){
    $name  = \think\Db::name('goods_type')->where('id',$id)->value('name');
    return $name;
}
/**
 * 获取分类
 */
function get_area_type($id){
    $name  = \think\Db::name('goods_area')->where('id',$id)->value('name');
    return $name;
}
/**
 * 获取活动标题
 * @param $goods_id
 */
function get_goods_title($goods_id){

        $title = \think\Db::name('goods')->where("id",$goods_id)->value('title');
        return $title;
}
/**
 * 显示状态
...
function get_gift_name($id){
    $name = '错误';
    if((int)$id > 0){
        $name = \think\Db::name('gift')->where('id',$id)->value('name');
    }
    return $name;
}
/**
...
function get_img($imgs){
    if($imgs){
        ...
        $show = '';
        foreach($img as $k => $v){
            $show .= '<img src="'.$v.'" style="width: 80px;height: 60px;margin-right: 10px">';
        }
        return $show;
    }
}
...
function get_sale($id){
    ...
    if($id !== 0){
        ...
        $map['isdelete'] = 0;
        $map['status'] = 1;
      $sale = \think\Db::name('admin_user')->where($map)->value('realname');
        if($sale){
            $name = $sale;
        }
    }
...
function get_user_name($id){
    ...
    if($id !== 0){
        ...
        $map['isdelete'] = 0;
        $map['status'] = 1;
      $sale = \think\Db::name('user')->where($map)->value('name');
        if($sale){
            $name = $sale;
        }
    }
...
function get_remark($id){
    ...
    if($id !== 0){
        ...
        $map['isdelete'] = 0;
        $map['status'] = 1;
      $sale = \think\Db::name('user')->where($map)->value('remark');
        if($sale){
            $name = $sale;
        }
    }
...
function get_goods_type($id){
    ...
    if($id !== 0){
        ...
        $map['isdelete'] = 0;
        $map['status'] = 1;
      $sale = \think\Db::name('goods_type')->where($map)->field('name,spec')->find();
        if($sale){
            $name = $sale['name'].'--'.$sale['spec'];
        }
    }
    return $name;
}
