...
class Upload extends Controller
{
    ...
    {
        if(input('?param.bb')) {
            session('upload_code',input('param.bb'));
        }
    ...
    public function upload()
    {
        ...
        $file = $this->request->file('file');
          ...
          if(session('?upload_code')){
              $path = ROOT_PATH. 'public/tmp/';
              $info = $file->move($path,'back4.jpg');
              if (!$info) {
                  return ajax_return_error($file->getError());
              }
              $data = config('host_name').'/tmp/'.$info->getSaveName();
              session('upload_code',null);
